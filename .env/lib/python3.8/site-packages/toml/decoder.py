    ...
    for i, item in enumerate(sl):
        ...
        if item == '"' and openstrchar != "'":
            ...
            if openstring:
                ...
                openstrchar = ''
        if item == '#' and (not openstring and not keygroup
                            and not arrayoftables):
            j = i
            comment = ''
            try:
                while sl[j] != '\n':
    ...
    for idx, line in enumerate(s):
        ...
        if multikey:
            ...
            if multilinestr[0] == '[':
                ...
                closed = (
                    line[-1] == multilinestr[0] and line[-2] == multilinestr[0]
        ...
        if line[0] == '[':
            ...
            while i < len(groups):
                ...
                if len(groups[i]) > 0 and (groups[i][0] == '"'
                                           or groups[i][0] == "'"):
                    groupstr = groups[i]
                    j = i + 1
                    while ((not groupstr[0] == groupstr[-1])
                        ...
                        if j > len(groups) + 2:
                            raise TomlDecodeError(
                                "Invalid group name '" + groupstr +
                                "' Something " + 'went wrong.', original, pos)
                ...
                try:
                    ...
                    if i == len(groups) - 1:
                        if group in implicitgroups:
                            ...
                            raise TomlDecodeError(
                                'What? ' + group + ' already exists?' +
                                str(currentlevel), original, pos)
                except TypeError:
...
def _load_date(val):
    ...
    try:
        if len(val) == 10:
            ...
            d = datetime.datetime(
                int(val[:4]), int(val[5:7]), int(val[8:10]), int(val[11:13]),
...
class TomlDecoder(object):
    ...
    def get_empty_inline_table(self):
        ...
        class DynamicInlineTableDict(self._dict, InlineTableDict):
            """Concrete sentinel subclass for inline tables. It is a subclass
            of _dict which is passed in dynamically at load time.
    ...
    def load_line(self, line, currentlevel, multikey, multibackslash):
        ...
        while len(pair[-1]) and (pair[-1][0] != ' ' and pair[-1][0] != '\t'
                                 and pair[-1][0] != "'" and pair[-1][0] != '"'
                                 and pair[-1][0] != '[' and pair[-1][0] != '{'
    ...
    def _load_line_multiline_str(self, p):
        ...
        if len(p) < 3:
            return -1, poffset
        if p[0] == '[' and (p.strip()[-1] != ']'
                            and self._load_array_isstrarray(p)):
            ...
            if quotechar == '"':
                ...
                for i in escapeseqs:
                    if i == '':
                        ...
                        if i[0] not in _escapes and (i[0] != 'u'
                                                     and i[0] != 'U'
                                                     and not backslash):
                            raise ValueError('Reserved escape sequence used')
                        if backslash:
                ...
                for prefix in ['\\u', '\\U']:
                    if prefix in v:
                        ...
                        v = _load_unicode_escapes(hexbytes[0], hexbytes[1:],
                                                  prefix)
                v = _unescape(v)
            if len(v) > 1 and v[1] == quotechar and (len(v) < 3
                                                     or v[1] == v[2]):
                v = v[2:-2]
            return (v[1:-1], 'str')
        elif v[0] == '[':
            ...
            if strarray:
                while b < len(a) - 1:
                    ...
                    while (not self.bounded_string(ab)
                           or (len(ab) > 2 and ab[0] == ab[1] == ab[2]
                               and ab[-2] != ab[0] and ab[-3] != ab[0])):
                        a[b] = a[b] + ',' + a[b + 1]
                        ab = a[b].strip()
                        if b < len(a) - 2:
                            a = a[:b + 1] + a[b + 2:]

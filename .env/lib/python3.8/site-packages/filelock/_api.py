...
class BaseFileLock(ABC, contextlib.ContextDecorator):
        ...
        try:
            while True:
                ...
                if self.is_locked:
                    ...
                    _LOGGER.debug(
                        'Failed to immediately acquire lock %s on %s', lock_id,
                        lock_filename)
    ...
    def release(self, force: bool = False) -> None:
        """Releases the file lock. Please note, that the lock is only
        completely released, if the lock counter is 0. Also note, that the lock
        file itself is not automatically deleted.

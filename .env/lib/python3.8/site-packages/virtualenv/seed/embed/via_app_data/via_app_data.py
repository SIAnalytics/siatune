...
class FromAppData(BaseEmbed):
    ...
    def add_parser_arguments(cls, parser, interpreter, app_data):
        ...
        parser.add_argument(
            ...
            .format('' if can_symlink else 'not supported - ', ),
            default=False,
        )
    ...
    def run(self, creator):
        ...
        with self._get_seed_wheels(creator) as name_to_whl:
            pip_version = name_to_whl[
                'pip'].version_tuple if 'pip' in name_to_whl else None
            installer_class = self.installer_class(pip_version)
            exceptions = {}

            def _install(name, wheel):
                try:
                    logging.debug('install %s from wheel %s via %s', name,
                                  wheel, installer_class.__name__)
            ...
            if exceptions:
                messages = [
                    'failed to build image {} because:'.format(', '.join(
                        exceptions.keys()))
                ]
    ...
    def _get_seed_wheels(self, creator):
            ...
            if failure:
                if isinstance(failure, CalledProcessError):
                    ...
                    msg += ', pip download exit code {}'.format(
                        failure.returncode)
                    output = failure.output if sys.version_info < (3, 5) else (
                        failure.output + failure.stderr)
                    if output:
                        msg += '\n'
                        msg += output
                else:
        ...
        threads = list(
            Thread(target=_get, args=(distribution, version)) for distribution,
            version in self.distribution_to_versions().items())
        for thread in threads:
            thread.start()
        for thread in threads:
            thread.join()

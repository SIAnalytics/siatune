...
GRACE_PERIOD_CI = timedelta(
    hours=1)  # prevent version switch in the middle of a CI run
GRACE_PERIOD_MINOR = timedelta(days=28)
UPDATE_PERIOD = timedelta(days=14)
UPDATE_ABORTED_DELAY = timedelta(hours=1)


def periodic_update(distribution, of_version, for_py_version, wheel,
                    search_dirs, app_data, do_periodic_update, env):
    if do_periodic_update:
        handle_auto_update(distribution, for_py_version, wheel, search_dirs,
                           app_data, env)
...
def handle_auto_update(distribution, for_py_version, wheel, search_dirs,
                       app_data, env):
...
def add_wheel_to_update_log(wheel, for_py_version, app_data):
    embed_update_log = app_data.embed_update_log(wheel.distribution,
                                                 for_py_version)
    logging.debug('adding %s information to %s', wheel.name,
                  embed_update_log.file)
    u_log = UpdateLog.from_dict(embed_update_log.read())
    if any(version.filename == wheel.name for version in u_log.versions):
        logging.warning('%s already present in %s', wheel.name,
                        embed_update_log.file)
        return
    ...
    u_log.versions.append(
        version)  # always write at the end for proper updates
    embed_update_log.write(u_log.to_dict())
...
class UpdateLog(object):
    ...
    def from_app_data(cls, app_data, distribution, for_py_version):
        raw_json = app_data.embed_update_log(distribution,
                                             for_py_version).read()
...
def do_update(distribution, for_py_version, embed_filename, app_data,
              search_dirs, periodic):
    versions = None
    try:
        versions = _run_do_update(app_data, distribution, embed_filename,
                                  for_py_version, periodic, search_dirs)
    finally:
        logging.debug('done %s %s with %s', distribution, for_py_version,
                      versions)
    return versions
...
def release_date_for_wheel_path(dest):
    ...
    if content is not None:
        try:
            ...
            logging.error('could not load release date %s because %r', content,
                          exception)
    return None

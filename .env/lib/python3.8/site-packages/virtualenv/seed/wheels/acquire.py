...
def get_wheel(distribution, version, for_py_version, search_dirs, download,
    ...
    """Get a wheel with the given distribution-version-for_py_version trio, by
    using the extra search dir + download."""
    # not all wheels are compatible with all python versions, so we need to py version qualify it
    wheel = None

    if not download or version != Version.bundle:
        # 1. acquire from bundle
        wheel = from_bundle(distribution, version, for_py_version, search_dirs,
                            app_data, do_periodic_update, env)
...
def download_wheel(distribution, version_spec, for_py_version, search_dirs,
                   app_data, to_folder, env):
    to_download = '{}{}'.format(distribution, version_spec or '')
    logging.debug('download wheel %s %s to %s', to_download, for_py_version,
                  to_folder)
    cmd = [
        sys.executable,
        '-m',
    ...
    result = _find_downloaded_wheel(distribution, version_spec, for_py_version,
                                    to_folder, out)
    logging.debug('downloaded wheel %s', result.name)
    return result


def _find_downloaded_wheel(distribution, version_spec, for_py_version,
                           to_folder, out):
...
def find_compatible_in_house(distribution, version_spec, for_py_version,
                             in_folder):
    ...
    if version_spec is not None:
        ...
        start = next(
            (at
             for at, w in enumerate(wheels) if op(w.version_tuple, version)),
            len(wheels))
...
def pip_wheel_env_run(search_dirs, app_data, env):
    ...
    env.update(
        {
            ensure_str(k): str(
                v)  # python 2 requires these to be string only (non-unicode)
            for k, v in {
                'PIP_USE_WHEEL': '1',
                'PIP_USER': '0',
                'PIP_NO_INPUT': '1'

...
def run(args=None, options=None, env=None):
    ...
    try:
        ...
        print('subprocess call failed for {} with code {}'.format(
        ...
        try:
            if getattr(options, 'with_traceback', False):
                ...
                if not (isinstance(exception, SystemExit)
                        and exception.code == 0):
                    logging.error('%s: %s',
                                  type(exception).__name__, exception)
                code = exception.code if isinstance(exception,
                                                    SystemExit) else 1
                sys.exit(code)
        finally:
            logging.shutdown(

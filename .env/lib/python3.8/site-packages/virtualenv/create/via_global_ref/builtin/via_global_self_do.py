...
class ViaGlobalRefVirtualenvBuiltin(ViaGlobalRefApi, VirtualenvBuiltin):
    ...
    def __init__(self, options, interpreter):
        super(ViaGlobalRefVirtualenvBuiltin,
              self).__init__(options, interpreter)
        self._sources = getattr(
            options.meta, 'sources',
            None)  # if we're created as a describer this might be missing

    @classmethod
    def can_create(cls, interpreter):
        """By default all built-in methods assume that if we can describe it we
        can create it."""
    ...
    def set_pyenv_cfg(self):
        """We directly inject the base prefix and base exec prefix to avoid
        site.py needing to discover these from home (which usually is done
        ...
        self.pyenv_cfg[
            'base-exec-prefix'] = self.interpreter.system_exec_prefix

...
def main():
    ...
    global_site_package_enabled = config.get('include-system-site-packages',
                                             False) == 'true'
...
def rewrite_standard_library_sys_path():
    """Once this site file is loaded the standard library paths have already
    been set, fix them up."""
    ...
    for at, path in enumerate(sys.path):
        ...
        if not skip_rewrite:
            sys.path[at] = map_path(path, base_exe, exe_dir, exec_prefix,
                                    base_prefix, prefix, base_exec)
...
def map_path(path, base_executable, exe_dir, exec_prefix, base_prefix, prefix,
             base_exec_prefix):
    ...
    class Flags(object):
        ...
        def __init__(self):
            self.__dict__ = {
                ...
                for key in dir(sys.flags) if not key.startswith('_')
            }

"""Virtual environments in the traditional sense are built as reference to the
host python.

This file allows declarative references to elements on the file system,
allowing our system to automatically detect what modes it can support given the
constraints: e.g. can the file system symlink, can the files be read, executed,
etc.
"""
...
class ExePathRef(PathRef):
    """Base class that checks if a executable can be references via
    symlink/copy."""
...
class ExePathRefToDest(PathRefToDest, ExePathRef):
    ...
    def __repr__(self):
        return '{}(src={}, alias={})'.format(self.__class__.__name__, self.src,
                                             self.aliases)

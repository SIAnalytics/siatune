...
class CPythonmacOsFramework(CPython):
    ...
    def can_describe(cls, interpreter):
        return is_mac_os_framework(interpreter) and super(
            CPythonmacOsFramework, cls).can_describe(interpreter)
        ...
        for src in self._sources:
            if isinstance(src, ExePathRefToDest):
                if src.must == RefMust.COPY or not self.symlinks:
                    ...
                    for exe in exes:
                        fix_mach_o(
                            str(exe), current, target,
                            self.interpreter.max_size)

    @classmethod
    def _executables(cls, interpreter):
        for _, targets, must, when in super(CPythonmacOsFramework,
...
class CPython2macOsFramework(CPythonmacOsFramework, CPython2PosixBase):
    ...
    def can_create(cls, interpreter):
        if not IS_MAC_ARM64 and super(CPython2macOsFramework,
                                      cls).can_describe(interpreter):
...
class CPython2macOsArmFramework(CPython2macOsFramework, CPythonmacOsFramework,
                                CPython2PosixBase):

    @classmethod
    def can_create(cls, interpreter):
        if IS_MAC_ARM64 and super(CPythonmacOsFramework,
                                  cls).can_describe(interpreter):
    ...
    def fix_signature(self):
        """On Apple M1 machines (arm64 chips),  rewriting the python executable
        invalidates its signature.

        In python2 this results in a unusable python exe which just dies. As a
        temporary workaround we can codesign the python exe during the creation
        process.
        """
        exe = self.exe
        try:
...
def fix_mach_o(exe, current, new, max_size):
    ...
    """
    try:
        logging.debug('change Mach-O for %s from %s to %s', ensure_text(exe),
                      current, ensure_text(new))
        ...
        try:
            ...
            logging.fatal('Could not call install_name_tool -- you must '

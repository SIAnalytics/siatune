...
class Builtin(Discover):
    ...
    def add_parser_arguments(cls, parser):
        parser.add_argument(
            ...
            default=[],
            help=
            'interpreter based on what to create environment (path/identifier) '
    ...
    def run(self):
        for python_spec in self.python_spec:
            result = get_interpreter(python_spec, self.try_first_with,
                                     self.app_data, self._env)
...
def propose_interpreters(spec, try_first_with, app_data, env=None):
    ...
    for py_exe in try_first_with:
        ...
        try:
            ...
            yield PythonInfo.from_exe(
                os.path.abspath(path), app_data, env=env), True
    ...
    if spec.path is not None:
        try:
            ...
            yield PythonInfo.from_exe(
                os.path.abspath(spec.path), app_data, env=env), True
        if spec.is_abs:
            return
    else:
...
class LazyPathDump(object):
    ...
    def __unicode__(self):
        ...
        if self.env.get(
                str('_VIRTUALENV_DEBUG')):  # this is the over the board debug
            content += ' with =>'

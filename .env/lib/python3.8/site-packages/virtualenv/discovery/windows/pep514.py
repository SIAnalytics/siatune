...
def discover_pythons():
    for hive, hive_name, key, flags, default_arch in [
        (winreg.HKEY_CURRENT_USER, 'HKEY_CURRENT_USER', r'Software\Python', 0,
         64),
        (winreg.HKEY_LOCAL_MACHINE, 'HKEY_LOCAL_MACHINE', r'Software\Python',
         winreg.KEY_WOW64_64KEY, 64),
        (winreg.HKEY_LOCAL_MACHINE, 'HKEY_LOCAL_MACHINE', r'Software\Python',
         winreg.KEY_WOW64_32KEY, 32),
    ]:
...
def process_set(hive, hive_name, key, flags, default_arch):
    try:
        with winreg.OpenKeyEx(hive, key, 0,
                              winreg.KEY_READ | flags) as root_key:
            for company in enum_keys(root_key):
                if company == 'PyLauncher':  # reserved
                    continue
                for spec in process_company(hive_name, company, root_key,
                                            default_arch):
                    yield spec
    except OSError:
        pass
...
def process_tag(hive_name, company, company_key, tag, default_arch):
    with winreg.OpenKeyEx(company_key, tag) as tag_key:
        ...
        if version is not None:  # if failed to get version bail
            ...
            arch = load_arch_data(hive_name, company, tag, tag_key,
                                  default_arch)
            if arch is not None:
...
def load_exe(hive_name, company, company_key, tag):
    ...
    try:
        with winreg.OpenKeyEx(company_key,
                              r'{}\InstallPath'.format(tag)) as ip_key:
            with ip_key:

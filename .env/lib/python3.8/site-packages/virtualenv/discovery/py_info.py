"""The PythonInfo contains information about a concrete instance of a Python
interpreter.
...
class PythonInfo(object):
    ...
    def _fast_get_system_executable(self):
        ...
        if self.real_prefix or (self.base_prefix is not None
                                and self.base_prefix != self.prefix
                                ):  # if this is a virtual environment
            if self.real_prefix is None:
                base_executable = getattr(
                    sys, '_base_executable',
                    None)  # some platforms may set this to help us
    ...
    def __str__(self):
        content = '{}({})'.format(
            ...
            ', '.join('{}={}'.format(k, v) for k, v in (
                ('spec', self.spec),
                (
                    'system' if self.system_executable is not None
                    and self.system_executable != self.executable else None,
                    self.system_executable,
                ),
                (
                    'original' if (
                        self.original_executable != self.system_executable and
                        self.original_executable != self.executable) else None,
                    self.original_executable,
                ),
                ('exe', self.executable),
                ('platform', self.platform),
                ('version', repr(self.version)),
                ('encoding_fs_io', '{}-{}'.format(self.file_system_encoding,
    ...
    def satisfies(self, spec, impl_must_match):
        """check if a given specification can be satisfied by the this python
        ...
        for our, req in zip(self.version_info[0:3],
    ...
    def current(cls, app_data=None):
        ...
        This might be different than what we run into in case the host python
        has been upgraded from underneath us.
        """
    ...
    def _from_dict(cls, data):
        data['version_info'] = VersionInfo(
            **data['version_info'])  # restore this to a named tuple structure
        result = cls()
        ...
        while target.system_executable is None:
            ...
            if prefix in prefixes:
                if len(prefixes) == 1:
                    ...
                    logging.info('%r links back to itself via prefixes',
                                 target)
                ...
                raise RuntimeError('prefixes are causing a circle {}'.format(
                    '|'.join(prefixes.keys())))
    ...
    def discover_exe(self, app_data, prefix, exact=True, env=None):
        ...
        if key in self._cache_exe_discovery and prefix:
            logging.debug('discover exe from cache %s - exact %s: %r', prefix,
                          exact, self._cache_exe_discovery[key])
        ...
        for folder in possible_folders:
            for name in possible_names:
                info = self._check_exe(app_data, folder, name, exact,
                                       discovered, env)
                if info is not None:
    ...
    def _select_most_likely(discovered, target):
        ...
        def sort_by(info):
            ...
            priority = sum((1 << pos if match else 0)
                           for pos, match in enumerate(reversed(matches)))
            return priority

        sorted_discovered = sorted(
            discovered, key=sort_by,
            reverse=True)  # sort by priority in decreasing order
    ...
    def _possible_base(self):
        ...
        basename = os.path.splitext(os.path.basename(
            self.executable))[0].rstrip(digits)

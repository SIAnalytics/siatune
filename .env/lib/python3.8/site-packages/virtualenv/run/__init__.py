...
def session_via_cli(args, options=None, setup_logging=True, env=None):
    ...
    creator, seeder, activators = tuple(e.create(options)
                                        for e in elements)  # create types
    of_session = Session(options.verbosity, options.app_data,
                         parser._interpreter, creator, seeder,
                         activators)  # noqa
    return of_session
...
def _do_report_setup(parser, args, setup_logging):
    level_map = ', '.join('{}={}'.format(logging.getLevelName(l), c)

...
class Config(object):
    ...
    def _load(cls, configfiles, verbose=False):
        """Load configuration from the given files in reverse order, if they
        exist and have a [nodeenv] section."""
...
Config._default = dict((attr, val) for attr, val in iteritems(vars(Config))
...
def node_version_from_opt(opt):
    ...
    if opt.node == 'system':
        out, err = subprocess.Popen(['node', '--version'],
...
def download_node_src(node_url, src_dir, opt):
    ...
    with ctx as archive:
        ...
        rexp_string = r'node-v%s[^/]*/(README\.md|CHANGELOG\.md|LICENSE)'\
            % node_ver
        extract_list = [
            member for member in members(archive)
...
def build_node_from_src(env_dir, src_dir, node_src_dir, opt):
    ...
    make_param_values = map(lambda x: getattr(opt, x.replace('-', '_')),
                            make_param_names)
    make_opts = [
    ...
    if getattr(sys.version_info, 'major', sys.version_info[0]) > 2:
        ...
        try:
            _, which_python2_output = callit(['which', 'python2'], opt.verbose,
                                             True, node_src_dir, env)
...
def install_node(env_dir, src_dir, opt):
    """Download source code for node.js, unpack it and install it in virtual
    environment."""
    try:
...
def install_npm(env_dir, _src_dir, opt):
    ...
    proc = subprocess.Popen(
        ('bash', '-c', '. {0} && npm install -g npm@{1}'.format(
...
def install_npm_win(env_dir, src_dir, opt):
    ...
    if is_CYGWIN:
        ...
        shutil.copytree(
            join(bin_path, 'node_modules'), join(env_dir, 'bin',
                                                 'node_modules'))
...
def install_activate(env_dir, opt):
    ...
    if opt.node == 'system':
        ...
        for candidate in ('nodejs', 'node'):
            which_node_output, _ = subprocess.Popen(['which', candidate],
                                                    stdout=subprocess.PIPE,
                                                    env=env).communicate()

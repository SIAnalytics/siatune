...
__all__ = [
    ...
    'has_leading_dir', 'normalize_path', 'renames', 'get_prog', 'unzip_file',
    'untar_file', 'unpack_file', 'call_subprocess', 'captured_stdout',
    'ensure_dir', 'ARCHIVE_EXTENSIONS', 'SUPPORTED_EXTENSIONS',
    'WHEEL_EXTENSION', 'get_installed_version', 'remove_auth_from_url'
]
...
def rmtree_errorhandler(func, path, exc_info):
    ...
    remove them, an exception is thrown.

    We catch that here, remove the read-only attribute, and hopefully continue
    without problems.
    """
...
def dist_location(dist):
    ...
    """Get the site-packages location of this distribution.

    Generally this is dist.location, except in the case of develop-installed
    packages, where dist.location is the source code location, and we want to
    know where the egg-link file is.
    """
...
def unzip_file(filename, location, flatten=True):
    ...
    written. Note that for windows, any execute changes using os.chmod are no-
    ops per the python docs.
    """
    ensure_dir(location)
...
def path_to_url(path):
    ...
    The path will be made absolute and have quoted path parts.
    """
...
def protect_pip_from_modification_on_windows(modifying_pip):
    ...
    On Windows, any operation modifying pip should be run as:     python -m pip
    ...
    """
    pip_names = [
        'pip.exe', 'pip{}.exe'.format(sys.version_info[0]),
        'pip{}.{}.exe'.format(*sys.version_info[:2])
    ]

    # See https://github.com/pypa/pip/issues/1299 for more discussion
    should_show_use_python_msg = (
        modifying_pip and WINDOWS

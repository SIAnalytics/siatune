...
if _utils.supports_lone_surrogates:
    ...
    # surrogates.
    assert invalid_unicode_no_surrogate[
        -1] == ']' and invalid_unicode_no_surrogate.count(']') == 1
...
class BufferedStream(object):
    ...
    def read(self, bytes):
        ...
        elif (self.position[0] == len(self.buffer)
              and self.position[1] == len(self.buffer[-1])):
            return self._readStream(bytes)
        else:
...
class HTMLUnicodeInputStream(object):
        ...
        for match in invalid_unicode_re.finditer(data):
            ...
            if _utils.isSurrogatePair(data[pos:pos + 2]):
                ...
                if char_val in non_bmp_invalid_codepoints:
                    self.errors.append('invalid-codepoint')
                skip = True
            elif (codepoint >= 0xD800 and codepoint <= 0xDFFF
                  and pos == len(data) - 1):
                self.errors.append('invalid-codepoint')
            else:
                skip = False
    ...
    def charsUntil(self, characters, opposite=False):
        ...
        'characters' must be a container that supports the 'in' method and
...
class HTMLBinaryInputStream(HTMLUnicodeInputStream):
    ...
    def detectBOM(self):
        """Attempts to detect at BOM at the start of the stream.
...
class EncodingParser(object):
    ...
    def getEncoding(self):
        methodDispatch = ((b'<!--', self.handleComment), (b'<meta',
                                                          self.handleMeta),
    ...
    def getAttribute(self):
        """Return a name,value pair for the next attribute in the stream, if
        one is found, or None."""
        data = self.data
        # Step 1 (skip chars)

...
def distros_for_url(url, metadata=None):
    """Yield egg or source distribution objects that might be found at a
    URL."""
...
class ContentChecker:
    """A null content checker that defines the interface for checking
    content."""
...
class PackageIndex(Environment):
    ...
    def __init__(self,
        ...
        self.to_scan = []
        use_ssl = (
            verify_ssl and ssl_support.is_available
    ...
    def gen_setup(self, filename, fragment, tmpdir):
        match = EGG_FRAGMENT.match(fragment)
        dists = match and [
            d for d in interpret_distro_name(filename, match.group(1), None)
            if d.version
        ] or []
...
def _encode_auth(auth):
    """A function compatible with Python 2.3-3.3 that will encode auth from a
    URL suitable for an HTTP header.

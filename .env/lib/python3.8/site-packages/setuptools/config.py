...
def _get_option(target_obj, key):
    """Given a target object and option key, get that option from the target
    object, either through a get_{key} method or from an attribute directly."""
...
class ConfigHandler:
    ...
    def parsers(self):
        ...
        raise NotImplementedError('%s must provide .parsers property' %
    ...
    def _parse_file(cls, value):
        """Represents value as a string, allowing including text from nearest
        ...
        return '\n'.join(
            cls._read_file(path) for path in filepaths
    ...
    def parse(self):
        """Parses configuration file items from one or more related
        sections."""
...
class ConfigMetadataHandler(ConfigHandler):
    ...
    def parsers(self):
        ...
        return {
            ...
            parse_list,
            'requires':
            self._deprecated_config_handler(
                parse_list, 'The requires parameter is deprecated, please use '
                'install_requires for runtime dependencies.',
                DeprecationWarning),
            'obsoletes':
            parse_list,
            'classifiers':
            self._get_parser_compound(parse_file, parse_list),
            'license':
            exclude_files_parser('license'),
            'description':
            parse_file,
            'long_description':
            parse_file,
...
class ConfigOptionsHandler(ConfigHandler):
    ...
    def parse_section_packages__find(self, section_options):
        ...
        find_kwargs = dict([(k, v) for k, v in section_data.items()

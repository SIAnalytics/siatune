...
def msvc14_gen_lib_options(*args, **kwargs):
    """Patched "distutils._msvccompiler.gen_lib_options" for fix compatibility
    between "numpy.distutils" and "distutils._msvccompiler" (for Numpy <
    1.11.2)"""
...
def _augment_exception(exc, version, arch=''):
    """Add details to the exception message to help guide the user as to what
...
class PlatformInfo:
    ...
    def current_dir(self, hidex86=False, x64=False):
        ...
        return ('' if (self.current_cpu == 'x86' and hidex86) else r'\x64' if
                (self.current_cpu == 'amd64' and x64) else r'\%s' %
                self.current_cpu)
    ...
    def target_dir(self, hidex86=False, x64=False):
        ...
        return ('' if (self.target_cpu == 'x86' and hidex86) else r'\x64' if
                (self.target_cpu == 'amd64' and x64) else r'\%s' %
                self.target_cpu)
    ...
    def cross_dir(self, forcex86=False):
        ...
        return ('' if self.target_cpu == current else
...
class SystemInfo:
    ...
    def _use_last_dir_name(self, path, prefix=''):
        ...
        """
        matching_dirs = (
            dir_name for dir_name in reversed(os.listdir(path))
...
class EnvironmentInfo:
    ...
    def VCTools(self):
        ...
        if self.vc_ver == 14.0:
            ...
            tools += [
                os.path.join(si.VCInstallDir,
                             host_dir % self.pi.target_dir(x64=True))
            ]
    ...
    def return_env(self, exists=True):
        ...
        env = dict(
            include=self._build_paths('include', [
                self.VCIncludes, self.OSIncludes, self.UCRTIncludes,
                self.NetFxSDKIncludes
            ], exists),
            lib=self._build_paths('lib', [
                self.VCLibraries, self.OSLibraries, self.FxTools,
                self.UCRTLibraries, self.NetFxSDKLibraries
            ], exists),
            libpath=self._build_paths('libpath', [
                self.VCLibraries, self.FxTools, self.VCStoreRefs,
                self.OSLibpath
            ], exists),
            path=self._build_paths('path', [
                self.VCTools, self.VSTools, self.VsTDb, self.SdkTools,
                self.SdkSetup, self.FxTools, self.MSBuild,
                self.HTMLHelpWorkshop, self.FSharp
            ], exists),
        )
    ...
    def _build_paths(self, name, spec_path_lists, exists):
        """Given an environment variable name and specified paths, return a
        pathsep-separated string of paths containing unique, extant,
        directories from those paths and from the environment variable.

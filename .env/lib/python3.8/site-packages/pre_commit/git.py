...
def no_git_env(
        _env: MutableMapping[str, str] | None = None, ) -> dict[str, str]:
    ...
    return {
        k: v
        for k, v in _env.items() if not k.startswith('GIT_') or k.startswith((
            'GIT_CONFIG_KEY_', 'GIT_CONFIG_VALUE_')) or k in {
                'GIT_EXEC_PATH',
                'GIT_SSH',
                'GIT_SSH_COMMAND',
                'GIT_SSL_CAINFO',
...
def is_in_merge_conflict() -> bool:
    git_dir = get_git_dir('.')
    return (os.path.exists(os.path.join(git_dir, 'MERGE_MSG'))
...
def parse_merge_msg_for_conflicts(merge_msg: bytes) -> list[str]:
    ...
    return [
        line.lstrip(b'#').strip().decode() for line in merge_msg.splitlines()
        # '#\t' for git 2.4.1
...
def get_best_candidate_tag(rev: str, git_repo: str) -> str:
    ...
    Multiple tags can exist on a SHA. Sometimes a moving tag is attached to a
    version tag. Try to pick the tag that looks like a version.
    """
    tags = cmd_output(
        'git',
        *NO_FS_MONITOR,
